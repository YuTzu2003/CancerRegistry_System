...
def validate_date_rules(row, alias_mapping):
    ...
    if class_field in std_row and first_visit in std_row and init_diag in std_row:
        class_val = str(std_row[class_field]).strip()
        if class_val.endswith('.0'): class_val = class_val[:-2]
        
        fv_val = parse_cancer_date(std_row[first_visit])
        id_val = parse_cancer_date(std_row[init_diag])
