...
def latest_commit_id(cursor, data_type):
    cursor.execute("""SELECT TOP 1 current_commit.CommitId FROM dbo.BranchCommits AS current_commit WHERE current_commit.DataType = ? 
        AND NOT EXISTS (SELECT 1 FROM dbo.BranchCommits AS child_commit WHERE child_commit.ParentCommitId = current_commit.CommitId AND child_commit.DataType = current_commit.DataType) 
...
def record_change(cursor, data_type, user_id, record_id, action, before=None, after=None, dataset=None):
    record_changes(cursor, data_type, user_id, [{ "record_id": record_id,"action": action,"before": before,"after": after,"dataset": dataset,}])

def record_changes(cursor, data_type, user_id, changes):
    if not changes:
        return
    ...
    with source.open("a", encoding="utf-8", newline="\n") as file:
        for change in changes:
            ...
            if change.get("dataset"):
                record["dataset"] = change["dataset"]
            file.write(json.dumps(record, ensure_ascii=False, default=str) + "\n")


def ensure_initial_commit(cursor, data_type, user_id, user_name):
    cursor.execute("UPDATE dbo.BranchCommits SET Info = N'建立初始版本' WHERE DataType = ? AND ParentCommitId IS NULL AND (Info IS NULL OR LTRIM(RTRIM(Info)) = '')",data_type,)
    cursor.execute("SELECT COUNT(*) FROM dbo.BranchCommits WHERE DataType = ?", data_type)
    if cursor.fetchone()[0]:
        return None
    commit_id = uuid4()
    cursor.execute("INSERT INTO dbo.BranchCommits (CommitId, ParentCommitId, CreatedByUserID, CreatedByName, Info, DataType, Action, ChangeCount, RevertUntil) VALUES (?, NULL, ?, ?, N'建立初始版本', ?, 'Commit', 0, DATEADD(DAY, 10, SYSDATETIME()))",str(commit_id), user_id, user_name, data_type,)
    return commit_id
...
def save_staging_commit(cursor, data_type, user_id, user_name, info, staged=None):
    ...
    commit_id = uuid4()
    cursor.execute("INSERT INTO dbo.BranchCommits (CommitId, ParentCommitId, CreatedByUserID, CreatedByName, Info, DataType, Action, ChangeCount, RevertUntil) VALUES (?, ?, ?, ?, ?, ?, 'Commit', ?, DATEADD(DAY, 10, SYSDATETIME()))",
...
def reverse_commits(cursor, data_type, target_id):
    ...
    while current_id and not commit_ids_equal(current_id, target_id):
        cursor.execute("SELECT ParentCommitId FROM dbo.BranchCommits WHERE CommitId = ? AND DataType = ?",current_id,data_type,)
