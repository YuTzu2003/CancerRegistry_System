(function() {
  ...
  function selectedCompareItems() {
    const previewState = comparisonPreview?.payload?.comparison_state;
    if (isComparisonPreview && Array.isArray(previewState?.compare_items)) {
      return [...new Set([
        ...previewState.compare_items,
        ...(previewState.stage_selection?.options || []).map(item => item.option),
        ...(comparisonPreview.payload.items || []).map(item => item.analysis_item)
      ])];
    }
  ...
  function selectedCompareStageOptions() {
    const previewSelection = comparisonPreview?.payload?.comparison_state?.stage_selection;
    if (isComparisonPreview && Array.isArray(previewSelection?.options)) {
      const analyzedItems = new Set((comparisonPreview.payload.items || []).map(item => item.analysis_item));
      const reports = [
        ...(comparisonPreview.payload.comparison_data?.analysis_data?.main?.stageReports || []),
        ...(comparisonPreview.payload.comparison_data?.analysis_data?.target?.stageReports || [])
      ];
      const options = [...previewSelection.options];
      reports.forEach(report => {
        if (analyzedItems.has(report.option) && !options.some(item => item.option === report.option)) {
          options.push({ system: report.staging_system, option: report.option, detailed: report.detailed === true });
        }
      });
      return {
        mode: previewSelection.mode || '',
        detailed: previewSelection.mode === 'detailed',
        options: options.filter(item => item?.system && item?.option)
      };
    }
  ...
  // 比較頁 LLM 敘述：組裝資料、快取、重新產生與任務預覽同步。
  ...
  function renderResult(data) {
    ...
    `).join('');
    tabs.onclick = event => {
      const button = event.target.closest('.compare-result-tab');
  ...
  function applyComparisonPreviewState() {
    ...
    const selectedItems = new Set([
      ...(state.compare_items || []),
      ...((state.stage_selection?.options || []).map((option) => option.option))
    ]);
  ...
  updateButtonState();
  // 圖表與統計 renderer。
  // 範圍：性別年齡、組織型態、診斷分類、AJCC 期別、療程、手術與差異摘要。
  // 共用文字、資料正規化、圖表與表格的基礎 helper。
  ...
  // 人口學與診斷統計：性別年齡、年齡中位數、可分析個案、組織型態與分類。
