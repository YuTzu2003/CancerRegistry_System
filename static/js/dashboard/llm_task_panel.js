(() => {
  const labels = { queued: '排隊中', running: '處理中', retrying: '準備重試', completed: '已完成', failed: '處理失敗' };
  ...
  const render = () => {
    ...
    panel.innerHTML = `<section class="llm-task-panel">
      ...
      <div class="llm-task-list">${displayed.length ? displayed.map((task) => {
        ...
        const elapsed = duration(value(task, 'CreatedAt'), ['completed', 'failed'].includes(status) ? value(task, 'UpdatedAt') : null);
        return `<article class="llm-task-row"><label class="llm-task-select" aria-label="選取任務 ${esc(id)}"><input class="task-cb" type="checkbox" value="${esc(id)}"></label><div class="llm-task-row__content"><div class="llm-task-row__heading"><h3>${esc(value(task, 'DocumentLabel') || '年報分析')}</h3></div><div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center; font-size:12px; margin-top:6px; color:var(--muted,#6b7280);"><span>建立時間：${esc(formatDate(value(task, 'CreatedAt')))}</span><span style="width:4px;height:4px;border-radius:50%;background:#d1d5db;flex-shrink:0;"></span><span>處理時間：${esc(elapsed)}</span><span style="width:4px;height:4px;border-radius:50%;background:#d1d5db;flex-shrink:0;"></span><span class="llm-task-status is-${esc(status)}">狀態：${esc(labels[status] || status)}</span></div></div><div class="llm-task-side"><span class="llm-task-id">${esc(id.slice(-5))}</span><div class="llm-task-side__bottom"><div class="llm-task-row__actions"><button class="btn btn-sm btn-outline-dark llm-preview" type="button" data-id="${esc(id)}">預覽</button><button class="btn btn-sm btn-outline-success llm-export" type="button" data-id="${esc(id)}">匯出</button><button class="btn btn-sm btn-danger llm-row-delete" type="button" data-id="${esc(id)}">刪除</button></div></div></div></article>`;
      }).join('') : `<div class="llm-task-empty">${tasks.length ? '找不到符合搜尋或狀態條件的任務。' : '目前尚無工作任務。'}</div>`}</div></section>`;
    const updateDeleteLabel = () => { const count = panel.querySelectorAll('.task-cb:checked').length; panel.querySelector('#btnBatchDelete').textContent = count ? `刪除勾選項目 (${count})` : '刪除勾選項目'; };
    ...
    panel.querySelector('#btnSelectAllTasks').addEventListener('click', () => { panel.querySelectorAll('.task-cb').forEach((item) => { item.checked = true; }); updateDeleteLabel(); });
    panel.querySelector('#btnClearTaskSelection').addEventListener('click', () => { panel.querySelectorAll('.task-cb').forEach((item) => { item.checked = false; }); updateDeleteLabel(); });
    panel.querySelectorAll('.task-cb').forEach((item) => item.addEventListener('change', updateDeleteLabel));
    ...
    panel.querySelectorAll('.llm-preview, .llm-export').forEach((button) => button.addEventListener('click', (event) => event.preventDefault()));
  };
  const load = async () => {
    try {
      ...
      if (response.ok && payload.success) {
        tasks = (payload.tasks || []).filter((task) => value(task, 'TaskType') === 'chart');
      }
    } catch (e) {}
    render();
